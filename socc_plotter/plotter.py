...
class Plotter:
    ...
    def __init__(
        ...
        img = np.zeros((self.screen_width // 2, self.screen_height, 3), dtype=np.uint8)
        ...
        self.window_2D.setGeometry(
            self.screen_origin[0], self.screen_origin[1], int(self.screen_width / 2), self.screen_height
        )
        ...
        car_colors = np.array([[0.5, 0.5, 0.5, 1] for i in range(len(car_faces))])
